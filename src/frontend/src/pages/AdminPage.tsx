...
import { deleteUser as deleteFirestoreUser } from "../utils/mlmIncome";
...
function FirestoreUsersSection() {
  ...
  async function handleDeleteUser(user: FirestoreUser) {
    const confirmed = window.confirm(
      `Delete ${user.name ?? user.phone ?? "this user"}? This will reverse all income and remove all their data.`,
    );
    if (!confirmed) return;
